...
public class RedisStore<T> {
  ...
  public T putIfAbsent(String collection, String key, T doc) {
    final ConcurrentMap<String, T> configurations = null;//redisson.getMap(collection);
  ...
  public T get(String collection, String key) {
    final ConcurrentMap<?, ?> configurations = null;//redisson.getMap(collection);
    return (T) configurations.get(key);
  }

  public Collection<T> entries(String collection) {
    final ConcurrentMap<?, T> configurations = null;//redisson.getMap(collection);

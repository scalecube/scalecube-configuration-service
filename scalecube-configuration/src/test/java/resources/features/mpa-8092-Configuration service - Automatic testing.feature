...
Feature: Integration tests for configuration service.
  ...
  #MPA-8092 (#2)
  Scenario: Fail to create the Repo upon access permission is restricted (Admin either Member API key)
    ...
    Then no repositories shouldn't be created and the for each of the requests user should get an error message: "role: "userId" not in role Owner"
  ...
  Scenario: Fail to create the Repo with the name which already exist (duplicate)
    ...
    Then no "repository" shouldn't be created and the user should get an error message: "repository": 'specified' name already exists"
  ...
  #MPA-8092 (#4.1)
  Scenario: Fail to create the Repo upon the Owner deleted the Organization with related "Owner" API key
  ...
  #MPA-8092 (#4.2) - logic will be implemented by Architect as the nature of the API key (token) is some expiration interim
  Scenario: Fail to create the Repo upon the Owner "token" was deleted from the Organization
    Given an organization "organizationId" with specified "name" and "email" already created with related "Owner" API key which is stored there
  ...
  #MPA-8092 (#2.3)
  Scenario: Fail to save (edit) the value for specific entity upon the valid API key was deleted
  ...
  #MPA-8092 (#3.4)
  Scenario: Fail to save (edit) the value for specific entity upon the valid API key with "Owner" role was deleted with relevant organization

  #MPA-8092 (#3.5)
  Scenario: Fail to save (edit) the value for specific entity upon the issuer of valid API key with "Admin" role was downgraded to "Member"

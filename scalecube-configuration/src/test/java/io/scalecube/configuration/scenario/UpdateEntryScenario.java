...
public class UpdateEntryScenario extends BaseScenario {
  ...
  void updateEntry(
    ...
    configurationService
        ...
        .then(
            configurationService.createEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName1, entryKey1, entryValue11)))
        .then(
            configurationService.updateEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName1, entryKey1, entryValue12)))
        .block(TIMEOUT);
    ...
    configurationService
        ...
        .then(
            configurationService.createEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName2, entryKey2, entryValue21)))
        .then(
            configurationService.updateEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName2, entryKey2, entryValue22)))
        .block(TIMEOUT);

    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName1, entryKey1)))
        .assertNext(
            entry -> {
    ...
    StepVerifier.create(
            configurationService.readEntry(
                new ReadEntryRequest(adminToken, repoName1, entryKey1, 1)))
        .assertNext(
            entry -> {
              assertEquals(entryKey1, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(
                new ReadEntryRequest(adminToken, repoName1, entryKey1, 2)))
        .assertNext(
            entry -> {
              assertEquals(entryKey1, entry.key(), "Fetched entry key");
              assertEquals(entryValue12, parse(entry.value()), "Fetched entry value");
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(memberToken, repoName2, entryKey2)))
        .assertNext(
            entry -> {
    ...
    StepVerifier.create(
            configurationService.readEntry(
                new ReadEntryRequest(adminToken, repoName2, entryKey2, 1)))
        .assertNext(
            entry -> {
              assertEquals(entryKey2, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(
                new ReadEntryRequest(adminToken, repoName2, entryKey2, 2)))
        .assertNext(
            entry -> {
              assertEquals(entryKey2, entry.key(), "Fetched entry key");
              assertEquals(entryValue22, parse(entry.value()), "Fetched entry value");
  ...
  void updateEntryDiffValues(
    ...
    configurationService
        ...
        .then(
            configurationService.updateEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName, entryKey, entryValue6)))
        .then(
            configurationService.updateEntry(
                new CreateOrUpdateEntryRequest(ownerToken, repoName, entryKey, entryValue7)))
        .block(TIMEOUT);
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName, entryKey, 3)))
        .assertNext(
            entry -> {
              assertEquals(entryKey, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName, entryKey, 4)))
        .assertNext(
            entry -> {
              assertEquals(entryKey, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName, entryKey, 5)))
        .assertNext(
            entry -> {
              assertEquals(entryKey, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName, entryKey, 6)))
        .assertNext(
            entry -> {
              assertEquals(entryKey, entry.key(), "Fetched entry key");
    ...
    StepVerifier.create(
            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName, entryKey, 7)))
        .assertNext(
            entry -> {
              assertEquals(entryKey, entry.key(), "Fetched entry key");
              assertEquals(entryValue7, parse(entry.value()), "Fetched entry value");
  ...
  void updateEntryByAdminAndMember(
    ...
    StepVerifier.create(
            configurationService.updateEntry(
                new CreateOrUpdateEntryRequest(memberToken , repoName, entryKey, entryValue3)))
        .expectErrorMessage(PERMISSION_DENIED)
        .verify();
  }

  //  @TestTemplate
  //  @DisplayName("#43 Scenario: Successful readEntry (specific version) from the related
  // Repository")
  ...
  //
  //    StepVerifier.create(
  //            configurationService.readEntry(new ReadEntryRequest(ownerToken, repoName,
  // entryKey)))
  //        .assertNext(
  //            entry -> {
  //              assertEquals(entryKey, entry.key(), "Fetched entry key");
  ...
  //
  //    StepVerifier.create(
  //            configurationService.readEntry(new ReadEntryRequest(adminToken, repoName, entryKey,
  // 3)))
  //        .assertNext(
  //            entry -> {
  //              assertEquals(entryKey, entry.key(), "Fetched entry key");
  ...
  //
  //  @TestTemplate
  //  @DisplayName("#44.1 (current error - Failed to decode data on message
  ...
  //        .expectErrorMessage(
  //            String.format(REPOSITORY_OR_ITS_KEY_NOT_FOUND_FORMATTER, repoNameNotExists,
  // entryKey))
  //        .verify();
  //  }
  //
  ...
  //
  //    StepVerifier.create(
  //            configurationService.readEntry(new ReadEntryRequest(token, repoName, entryKey)))
  ...
  //  @TestTemplate
  //  @DisplayName(
  //      "#47 Scenario: Fail to readEntry upon the \"Admin\" apiKey was deleted from the
  // Organization")
  ...
  //    StepVerifier.create(
  //            configurationService.readEntry(
  //                new ReadEntryRequest(tokenAdmin.key(), repoName, entryKey)))
  //        .expectErrorMessage(TOKEN_VERIFICATION_FAILED)
  //        .verify();
  //  }
  //
  ...
  //
  //    StepVerifier.create(
  //            configurationService.readEntry(new ReadEntryRequest(token, repository, "key")))
  //        .expectErrorMessage(TOKEN_VERIFICATION_FAILED)
  //        .verify();
  //  }
  //
  ...
  //        .block(TIMEOUT);
  //
  //    StepVerifier.create(configurationService.readEntry(new ReadEntryRequest(apiKey, "",
  // entryKey)))
  ...
  //        .expectErrorMessage(
  //            String.format(REPOSITORY_OR_ITS_KEY_NOT_FOUND_FORMATTER, repoName,
  // entryKeyNotExists))
  //        .verify();
  //  }
}
